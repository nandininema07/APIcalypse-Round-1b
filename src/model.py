...
class DocumentAnalyzer:
    ...
    def compute_relevance_scores(self, sections: List[Dict], persona: str, job_to_be_done: str, context_keywords: List[str]) -> List[Tuple[Dict, float]]:
        ...
        query_inputs = self.bert_tokenizer(query, return_tensors='pt', max_length=512, truncation=True, padding=True)
        with torch.no_grad():
            query_outputs = self.bert_model(**query_inputs)
            query_embedding = query_outputs.last_hidden_state.mean(dim=1).numpy()
        ...
        for section in sections:
            ...
            # Truncate and pad the input to max_length=512
            section_inputs = self.bert_tokenizer(section_text, return_tensors='pt', max_length=512, truncation=True, padding=True)
            with torch.no_grad():
